...
class SystemUser extends TRecord
{
    ...
    public function __construct($id = NULL)
    {
        ...
        parent::addAttribute('date_contract');
        parent::addAttribute('date_expiration');
        parent::addAttribute('days_contract');
        parent::addAttribute('obs');
        parent::addAttribute('last_try_login');
        parent::addAttribute('try_login');
        parent::addAttribute('login_block');
   ...
   /**
    ...
    {
        $user         = self::newFromLogin($login);
//         $user_control = new SystemUser($user->id)
        ...
        $login_block    = $user->login_block;
        $now            = new DateTime('NOW');
        
        if ($login_block == 1)
        {
            
            $last_try_login = new DateTime($user->last_try_login);
            $diff = $last_try_login->diff($now);
            $diff_minutes = ($diff->h * 60) + $diff->i;
            if ($diff_minutes < 1)
            {
                throw new Exception('Acesso bloqueado! Aguarde 15 minutos e tente novamente.');
            }
            else
            {
                $user->login_block = 0;
...
//                 $user->store();
        ...
        {    
            ...
            $try_login++;
            $user->try_login = $try_login;
            $msg = '';
            if ($try_login >= 5)
            {
                $user->login_block = 1;  
                $user->last_try_login = $now->format('Y-m-d H:i:s');
                $msg = ' - acesso bloqueado por 15 minutos por excesso de tentativas inválidas.';    
            }
            //$user->name= 'teste';
//             $user->store();
            UserControlLogin::store($user);
            throw new Exception('Senha incorreta' . $msg);
        }
